...
class RFRNetModel():
    ...
    #     return results[1:]
    def train(self,train_loader, save_path, vgg_path, finetune=False, iters=10000, batch_size=6, epochs=10, ssiter=0):
        ...
        def train_job(
            ...
            ret, mmask, fake, comp = self.buildnet(maskedimg, masks, images)
            loss, l1_loss = self.get_g_loss(ret, mmask, fake, comp)
            print("loss running")
            lr_scheduler = flow.optimizer.PiecewiseConstantScheduler([], [2e-4])
            # Set Adam optimizer
            flow.optimizer.Adam(lr_scheduler, do_bias_correction=False).minimize(loss)
    ...
    def test(self, test_loader, result_save_path):
        ...
        for items in test_loader:
            ...
            for k in range(comp_B.shape[0]):
                ...
                grid = flow.slice(masked_images, begin=[k, None, None, None],
                                  size=[1, None, None, None]) + 1 - flow.slice(masks, begin=[k, None, None, None],
                                                                               size=[1, None, None, None])
