...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='ap3dnlres50', 
...
input_image = tp.Numpy.Placeholder((args.train_batch , 3, args.seq_len, args.height,args.width))
...
def train_job(image:input_image,pids:input_pid
   )->Tuple[tp.Numpy,tp.Numpy,tp.Numpy,tp.ListNumpy]:
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids,training=True,resnetblock=getresnet)
    outputs,features=model.build_network(image)
        # combine hard triplet loss with cross entropy loss
    xent_loss=_CrossEntropyLoss(
        num_classes=dataset.num_train_pids
            ).forward(outputs,pids)
    htri_loss=TripletLoss(margin=args.margin,distance=args.distance).forward(features, pids)
    loss=xent_loss + htri_loss
    lr_scheduler=flow.optimizer.PiecewiseScalingScheduler(
            base_lr=args.lr,boundaries=args.stepsize,
            scale=[0.1,0.01,0.001]
        )
    flow.optimizer.Adam(lr_scheduler, do_bias_correction=False).minimize(loss)
    return loss,xent_loss,outputs,htri_loss
...
def train(epoch, trainset,dataset):
    ...
    batch=0
    indicies = [x for x in RandomIdentitySampler(train_id, args.num_instances)]
    for i in range(len(indicies) // batch_size):
        batch+=1
        try:
            train_batch=trainset.__getbatch__(indicies[i * batch_size:(i + 1) * batch_size])
        except:
            train_batch = trainset.__getbatch__(indicies[-batch_size:])
        data_time.update(time.time() - end)
        pids=train_batch[1].astype(np.float32)
        sum=np.sum(pids-pids[0])
        if(sum==0):
            continue
        
        loss,xent_loss,outputs,htri_loss=train_job(train_batch[0], train_batch[1].astype(np.float32))
        htri_loss=htri_loss[0]
        # statistics
        preds=np.argmax(outputs,axis=1)
        temp=np.sum(preds==pids)
        temp=np.float(temp)
        batch_corrects.update((temp/pids.shape[0]),pids.shape[0])
        batch_xent_loss.update(xent_loss.item(),pids.shape[0])
        batch_htri_loss.update(htri_loss.item(),pids.shape[0])
        batch_loss.update(loss.item(),pids.shape[0])
...
def main():
    ...
    trainset,queryset,galleryset=getDataSets(dataset)
    ...
    for epoch in range(start_epoch,args.max_epoch):
        ...
        if (epoch+1) >= args.start_eval and args.eval_step > 0 and (epoch+1) % args.eval_step == 0 or (epoch+1) == args.max_epoch:
            ...
            #save checkpoint
            fpath=osp.join(args.save_dir,'checkpoint_ep' + str(epoch+1))
            ...
            if is_best:
                ...
                if os.path.exists(best_fpath):
                    shutil.rmtree(best_fpath) 
                shutil.copytree(fpath, best_fpath)
