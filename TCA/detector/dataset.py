...
class Dataset(object):
    ...
    def __init__(self, dataset_type):
        ...
        self.anchors[0,...]/=self.strides[0]
        self.anchors[1,...]/=self.strides[1]   # relative the feature map
        self.anchor_per_scale = cfg.YOLO.ANCHOR_PER_SCALE
        self.max_bbox_per_scale = cfg.TRAIN.MAX_BBOX_PER_SCALE
    ...
    def __next__(self):
        ...
        if self.batch_count < self.num_batchs:
            while num < self.batch_size:
                ...
                num += 1
            self.batch_count += 1
            batch_image_ = np.transpose(batch_image, [0,3,1,2])  #[N, 3, H, W]
            batch_image = np.copy(batch_image_, order='C')
            return batch_image, batch_label_sbbox, batch_label_lbbox, batch_sbboxes, batch_lbboxes, \
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        for bbox in bboxes:
            ...
            if not exist_positive:
                ...
                xind = np.clip(xind, 0, self.train_output_sizes[best_detect] - 1)
                yind = np.clip(yind, 0, self.train_output_sizes[best_detect] - 1)
                # This will mitigate errors generated when the location computed by this is more the grid cell location.
                # e.g. For 52x52 grid cells possible values of xind and yind are in range [0-51] including both.
                # But sometimes the coomputation makes it 52 and then it will try to find that location in label array
                # which is not present and throws error during training.
