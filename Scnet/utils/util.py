...
viz = Visdom()
viz.line([[0., 0.]], [0], win='train', opts=dict(title='train-loss&acc', legend=['loss', 'acc']))
viz_val = Visdom()
        ...
        def callback(outputs):
            ...
            if (step + 1) % self.calculate_batches == 0:
                ...
                if self.loss_key:
                    ...
                    # 更新窗口图像
                    if self.desc == "train":
                        viz.line([[loss, acc]], [(epoch) * 200 + (step + 1)], win='train', update='append')


                else:
                    print(self.fmt.format(self.desc, epoch, step + 1, top_1_accuracy,
                                          top_k_accuracy, throughput), time.time())
                    viz_val.line([[top_1_accuracy]], [(epoch + 1) * (step + 1)], win='val', update='append')
